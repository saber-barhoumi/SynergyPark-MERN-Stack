...
const Header = () => {
  ...
  const displayName = (user as any)?.name || (user as any)?.username || (user as any)?.fullName || "Utilisateur";
  const displayEmail = (user as any)?.email || "-";
  const displayAvatar = (user as any)?.avatarUrl || (user as any)?.profileImage || "assets/img/profiles/avatar-12.jpg";
							...
							<div className="dropdown profile-dropdown">
								<Link to="#" className="dropdown-toggle d-flex align-items-center" data-bs-toggle="dropdown">
									<span className="avatar avatar-sm online">
										<ImageWithBasePath src={displayAvatar} alt="Img" className="img-fluid rounded-circle"/>
									</span>
								</Link>
										...
										<div className="card-header">
											<div className="d-flex align-items-center">
												<span className="avatar avatar-lg me-2 avatar-rounded">
													<ImageWithBasePath src={displayAvatar} alt="img"/>
												</span>
												<div>
													<h5 className="mb-0">{displayName}</h5>
													<p className="fs-12 fw-medium mb-0">{displayEmail}</p>
												</div>
											</div>
										</div>
										<div className="card-body">
